...
class Account extends CI_Controller {
	...
	public function changeMembership($user_id){
		$now = true;
		if($_POST['plan_code']=="daily" && $_POST['current_plan']=="daily"){
		    $result = array("nochange" => 1);
		} elseif ($_POST['plan_code']=="daily" && $_POST['current_plan'] != "daily"){
		   $this->update($user_id,false);
		} else {
			if ($now==false){
			  $this->update($user_id,false,0); // change subscription at renewal
			} else {
			  $this->update($user_id,true,0); // change subscription immediately
			}
