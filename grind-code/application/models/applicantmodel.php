...
class ApplicantModel extends CI_Model {
	...
	function getApplicants($status_id = null) {
		...
		$sql = "
			select
				user.id, user.date_added as date, user.first_name, user.last_name,applicant_why.why_me,
	...
	function insertApplicant($data) {
		...
		//handle membership stuff
        if($this->input->get_post('plan_id') || $this->input->get_post('plan_id')==='0'){
            $planId = $this->input->get_post('plan_id');
	    $planData = array(
                'user_id' => $userid,
                'plan_id' => $planId
            );
            $this->db->insert('applicant_plan',$planData);
        }
	...
	function createWordpressAccount($email, $first_name, $last_name) {
		...
		$temporaryPassword = $this->utilities->get_random_password();
