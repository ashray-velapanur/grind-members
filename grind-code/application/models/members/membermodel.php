...
class MemberModel extends CI_Model {
    ...
    function new_listing($num, $offset=NULL, $company_id=NULL, $user_id=NULL, $tag_id=NULL, $member_id=NULL){
        ...
        elseif($company_id) {
            $sql = "select user.id, user.first_name, user.last_name, third_party_user.network_id as linkedin_id, third_party_user.profile_picture as profile_picture, company.name as company, positions.designation as designation, (third_party_user.access_token IS NOT NULL) AS is_loggedin
                        from 
                            user 
    ...
    function signin($user_id, $location_id, $sign_in_method, $sign_in) {
        ...
        if ($result) {
            $memberData = $this->get_basicMemberData($this->member->id);
            $apiResult = CheckinApi::checkin($memberData['rfid'], $location_id);
