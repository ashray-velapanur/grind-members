...
class UserManagement extends CI_Controller {
	...
	public function search($value=NULL) {
        ...
        $planmodel = $this->load->model('billing/planmodel','',true);
			if (!isset($value)){
				if (isset($_POST)){
        ...
        $data['plans'] = array(
            'Conference Room','Paid Guest','Printing','Event','Other'
        );
        $data['locations'] = $this->locationmodel->wpGetLocationsList();
	...
	public function users($row=NULL) {
		...
		$config['base_url'] = site_url('/ci/admin/usermanagement/users/');
	...
	public function user($error=NULL) {
		...
		if ($this->uri->segment(4) != "") {
			...
			$data["billing_info"] = Recurly_BillingInfo::get($this->uri->segment(4));
		}
