...
class UserModel extends CI_Model {
    ...
    function init($id, $type = UserIdType::ID) {
		...
		$subscription = self::getSubscription($retval->id);
		if ($subscription) {
        ...
        try{
            $retval->billingInfo = Recurly_BillingInfo::get($retval->id);
        } catch(Recurly_NotFoundError $e){
            $retval->billingInfo = new Recurly_BillingInfo($retval->id);
        }
	...
	public function updateBillingFromWordpress($data) {
		...
		$billing_info = new Recurly_BillingInfo($acct->account_code);
        ...
        $billing_info->number = $data['credit_card']['number'];
        $billing_info->year = intval($data['credit_card']['year']);
        $billing_info->month = intval($data['credit_card']['month']);
        $billing_info->verification_value = $data['credit_card']['verification_value'];
    ...
    public function getUsers() {
		...
		foreach ($users as $user) {
			...
			$subscription = self::getSubscription($user->id);
			$planCode = "daily";
			$planName = "Daily";
			if($subscription) {
	...
	public function createRecurlySubscription($data) {
        ...
        if ($account===null)
        {
            $account = new Recurly_Account($this->userInfo->id);
    ...
    function getRecurlyAccount() {
        return Recurly_Account::get($this->userInfo->id);
    }
    ...
    function getRecurlySubscription($id) {
        if ($id == null)
        {
            return  self::getSubscription($this->userInfo->id);
        } else {
            return  self::getSubscription($id);
        }
    }
    ...
    public function makeSinglePayment($userId, $amount, $details = "", $isCredit = false) {
		...
		try {		
			...
			$account = Recurly_Account::get($userId);
			if (!$isCredit) {
                ...
                $credit->account_code = $account->account_code;
                $credit->currency = "USD";
    ...
    public function updateRecurlySubscription($data) {
        $subscription = self::getSubscription($this->userInfo->id);
        $subscription->plan_code = $data['plan_code'];
        $response = $subscription->updateImmediately();
		var_dump($response);
		exit;
    }
    ...
    public function createRecurlyAccount($id, $fname, $lname, $email) {
		...
		$account = new Recurly_Account($id);
	...
	public function createRecurlySubscription2(Recurly_Account $account, $planCode, $billingFirstName, $billingLastName, $billingAddr1, $billingAddr2 = "",
	 ...
	 $ccVerificationValue) {
        ...
        if (trim($ccVerificationValue) != "") $billing_info->verification_value = $ccVerificationValue;
		if (substr($ccNumber, 0, 5) != "*****") $billing_info->number = $ccNumber;
		$billing_info->ip_address = $_SERVER['REMOTE_ADDR'];
