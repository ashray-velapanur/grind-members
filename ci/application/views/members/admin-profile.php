...
?>
			...
			<div class="formContainer" id="your-account-container">
			  <input type="hidden" id="editaction" name="editaction" value = "<?=site_url('ci/members/profile/edit/')?>">
			...
			<div class="col membership">	
				<div class="profileSection" id="memberInfoSection">
					...
					<div class="itemContainer" id="location_idBlock">
                        <div class="roItemContainer">
                            <div class="roItem">
                                <span id="location_id_ro"><?= $currentLocation===null ? '<span class="placeholder">Member location</span>' : $currentLocation->name; ?></span>
                            </div>
                        </div>
                        <div class="editItemContainer clearfix">
                            <form id="location_id-form" rel="location_idBlock" class="clearfix">
                                <div class="floatingFormItem">
                                    <select class="text-input" name="location_id" id="location_id">
                                        <?php foreach($locations as $location): ?>
                                            <option value="<?php echo $location->id; ?>"<?php echo $currentLocation->id==$location->id ? " selected" : ''; ?>><?php echo $location->name; ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                    <label for="location_id" class="inputLabel">Location</label>
                                </div>
                                <div class="update">
                                    <input type="submit" value="Update" class="updateItem btn" rel="location_id"/>
                                </div>
                            </form>
                            <div class="cancelUpdate">
                              <a class="toggleit" rel="location_idBlock">Cancel</a>
                            </div>
                        </div>
                    </div>
					...
					<div id="post-pw-reset" class="pw-reset" style="display:none">
					            ...
					            Are you sure you want to reset this members password?<br /><br />
									<input type="hidden" id="resetaction" value="<?=site_url('ci/members/profile/reset_pw/')?>"/>
					...
					<div class="itemContainer" id="membershipTypeBlock">
						...
						<div class="editItemContainer clearfix">
							<? if ($member->membership_status_luid == MembershipStatus::INACTIVE_MEMBER ) {
								...
								echo "This membership has been deactivated.  <a href='".site_url('ci/member/profile/activate/'.$member->id)."'>Reactivate?</a>";
							} else {
						
								if($member->subscription_state != "active"){
									echo "There are pending changes on this account, manage membership in Recurly";
								} else {?>
							<form id="membership-form" action="<?=site_url("ci/billing/account/changeMembership/")?>" rel="membershipTypeBlock" class="clearfix">
								<div class="floatingFormItem">
									<?=$membershipChooser?>
...
<script src="<?=site_url('ci/js/admin_profile_temp.js')?>"></script>
