	...
	$(document).ready(function(){	
      ...
      disablePopup(".popup");
		...
		$("#popupCheckinSubmit").click(function(){
			...
			$.ajax({
			  type: "POST",
			  url: "<?=site_url('/ci/admin/usermanagement/checkin')?>",
			  data: dataString,
			  beforeSend: function() {
      ...
      if (valid) {
        ...
        dataString = "user_id=" + $("#Charge_user_id").val() + "&amount=" + $("#amount").val() + "&details=[" + $("#location").val() + '_' + $("#type").val() + '] ' + $("#comments").val() + "&dc=" + dc;
  			$.ajax({
  			  type: "POST",
  			  url: "<?=site_url('ci/admin/usermanagement/charge')?>",
  			  data: dataString,
  			  beforeSend: function() {
		      ...
		      <label for="location_id">Location</label>
					<select id="location_id" name="location_id">
						<? foreach($navlocations as $id => $location) { ?>
						    <?php $selected = ($navlocation_current==$id) ? 'selected="selected" ' : ''; ?>
							<option <?php echo $selected; ?>value="<?=$id ?>"><?=$location ?></option>
						<? } ?>
					</select>
				</li>
				<li>
		...
		<fieldset>
			<ul>
				...
				<li>
					...
					<?php
                       ...
                       foreach($locations as $id => $location){
                           $options[$location['charge_message']] = $location['name'];
                       }
					?>
					<?=form_dropdown('location',$options,null,'id="location"');?>
				</li>
				<li>
				    ...
				    <?php
				        ...
				        foreach($plans as $plan){
				            $options[strtoupper(str_replace(' ','',$plan))] = $plan;
				        }
				    ?>
				    <?=form_dropdown('type',$options,null,'id="type"');?>
				</li>
				...
				<li>
                    ...
                    <?=form_textarea(array("id" => "comments", "name" => "comments", "cols" => "30", "rows" => "2"));?>
                </li>
				<li>
